#  
#  Takes a user, and a list of recipients
...
class InitiateOrder
    ...
    # User is the user placing the order
    ...
    def initialize(user, payment_source, recipients, message, buyable, fee)
        @user = user
    ...
    def call
        ...
        begin
            ...
            Order.transaction do # Wrap everything in a transaction
                ...
                Log.create(log_type: Log::INFO, context: InitiateOrder.name, current_user: @user.id, message: "Initiating Order")
                ...
                @recipients.each{ |r| 
                    ...
                    recipient = if r.respond_to? :has_key? and r.has_key? "group"
                      ...
                      if @user.tester? and pn != @user.phone_number.to_s
                        raise "Test user can only place order for self"
                      end
                      
                      User.find_or_create_mobile_phone_account(pn, nil, nil).user 
                    else
                ...
                intent = if charge_amount_cents > 0
                    ...
                    Log.create(log_type: Log::INFO, context: InitiateOrder.name, current_user: @user.id, message: "Bypassing Payment Intent for order #{@order.id}")
                    Class.new do 
    ...
    def create_pass(recipient, message, buyable, order) 
        Log.create(log_type: Log::INFO, context: "InitiateOrder#create_pass", current_user: order.user.id, message: "Creating pending pass for order #{order.id}")
